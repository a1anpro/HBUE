...
class User(models.Model):
    ...
    userIcon = models.CharField(max_length=30)#用户头像路径，头像文件由用户上传，数据库只存用户头像路径
    userInfo = models.CharField(max_length=300)#用户简介
...
class Course(models.Model):
    ...
    likes = models.IntegerField()#推荐数
    unlikes = models.IntegerField()#不推荐数
    commentNum = models.IntegerField()#评论数
    callRate = models.IntegerField()#点名频率，通过评论计算得出
    passRate = models.IntegerField()#通过指标
    getRate = models.IntegerField()#收获指标

    def __str__(self):
        return self.className + '(' + self.teacherName + ')'
